 ...
 */
uint8_t* CxImage::GetBits(uint32_t row) const
{ 
	if (pDib){
		if (row) {
 ...
 */
bool CxImage::IsInside(int32_t x, int32_t y) const
{
 ...
 * \author [livecn]
 */
void CxImage::FreeMemory(void* memblock) const
{
	if (memblock)
		free(memblock);
}
