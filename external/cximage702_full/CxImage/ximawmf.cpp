...
HBITMAP CxImageWMF::CreateDIB(HDC hdc,long cx,long cy) const{
  BITMAPINFO bmi;
...
/////////////////////////////////////////////////////////////////////
    ...
    float const emfDpiX = (25.4f * emh.szlDevice.cx / emh.szlMillimeters.cx);
    float const emfDpiY = (25.4f * emh.szlDevice.cy / emh.szlMillimeters.cy);
    float const frameWidth =  (emh.rclFrame.right  - emh.rclFrame.left) / 100.f;
    float const frameHeight = (emh.rclFrame.bottom - emh.rclFrame.top) / 100.f;
    cx = std::round(emfDpiX * frameWidth / 25.4f);
    cy = std::round(emfDpiY * frameHeight / 25.4f);
	...
	if(!hBitmap){
		// try to create DIB
		hBitmap=CreateDIB(hDC0,cx,cy);
		if(!hBitmap){
			strcpy(info.szLastError, "CxImageWMF::Decode(): failed to CreateDIB()");
			DeleteEnhMetaFile(hMeta);
			return false;
		}
	}
