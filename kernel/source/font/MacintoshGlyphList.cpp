...
namespace{
  ...
  std::size_t const macinthosGlyphListSize = 258;
  std::string const macintoshGlyphList[macinthosGlyphListSize] = {
    ".notdef",
    ".null",
    "nonmarkingreturn",
    "space",
...
namespace kernel{ namespace font{
  ...
  void getUnicodeInMacintoshOrder(uint16_t glyphId, UnicodeInfo &uniInfo){
    ...
    if (glyphId < macinthosGlyphListSize)
      getUnicodeByName(macintoshGlyphList[glyphId], uniInfo);
  }

  std::string getGlyphNameInMacintoshOrder(uint16_t glyphId){
    if (glyphId < macinthosGlyphListSize)
      return macintoshGlyphList[glyphId];
  }
